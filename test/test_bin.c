...
{
    const uint8_t req[] = { TS_FETCH, 0x64, 0x4D, 0x65, 0x61, 0x73, // "Meas"
                            0xF7 };                                 // CBOR undefined
    const char resp_hex[] =
        "85 83 " // successful response: array with 3 elements
        "66 72 42 61 74 5F 56 "
...
void test_bin_patch_float_array(void)
{
    ...
    const uint8_t req[] = {
        TS_PATCH, 0x18, ID_CONF, 0xA1, 0x19, 0x70, 0x04, 0x82, 0xFA, 0x40, 0x11, 0x47, 0xAE, // 2.27
        0xFA,     0x40, 0x5C,    0x28, 0xF6                                                  // 3.44
    };
    const uint8_t resp_expected[] = { TS_STATUS_CHANGED };
...
void test_bin_fetch_float_array(void)
{
    ...
    const uint8_t req[] = { TS_FETCH, 0x18, ID_CONF, 0x19, 0x70, 0x04 };
    const uint8_t resp_expected[] = {
        TS_STATUS_CONTENT, 0x82, 0xFA, 0x40, 0x11, 0x47, 0xAE, 0xFA, 0x40, 0x5C, 0x28, 0xF6
    };
...
void test_bin_fetch_rounded_float(void)
{
    ...
    const uint8_t req[] = { TS_FETCH, 0x18, ID_CONF, 0x19, 0x60, 0x0A };
    const uint8_t resp_expected[] = { TS_STATUS_CONTENT, 0x08 };
...
void test_bin_patch_rounded_float(void)
{
    ...
    const uint8_t req[] = { TS_PATCH, 0x18, ID_CONF, 0xA1, 0x19, 0x60, 0x0A, 0x05 };
    const uint8_t resp_expected[] = { TS_STATUS_CHANGED };
...
{
    const uint8_t req[] = {
        TS_FETCH, 0x19, 0x70, 0x05,
        0xF7 // CBOR undefined
    };
    const uint8_t resp_expected[] = { 0x85, 0x02 };
...
void test_bin_fetch_record()
{
    ...
    const uint8_t resp_expected[] = { 0x85, 0xA3, 0x18, 0x81, 0x18, 0x7B,       // 123
                                      0x18, 0x82, 0xFA, 0x41, 0x68, 0x00, 0x00, // 14.5
                                      0x18, 0x83, 0x02 };
...
void test_bin_import_record(void)
{
    ...
    const uint8_t resp_expected[] = { 0x85, 0xA3, 0x18, 0x81, 0x18, 0x7C,       // 124
                                      0x18, 0x82, 0xFA, 0x41, 0x48, 0x00, 0x00, // 12.5
                                      0x18, 0x83, 0x05 };
...
void test_bin_exec(void)
{
    ...
    const uint8_t req[] = {
        TS_POST, 0x19, 0x50, 0x01, // object ID as endpoint
        0x80                       // empty array (no parameters)
    };
    const uint8_t resp_expected[] = { TS_STATUS_VALID };
...
void test_bin_patch_fetch_bytes(void)
{
    const uint8_t req_patch[] = {
        TS_PATCH, 0x18, ID_CONF, 0xA1, 0x19, 0x80, 0x00, 0x48,
        0x00,     0x01, 0x02,    0x03, 0x04, 0x05, 0x06, 0x07 // write 8 bytes
    };
    const uint8_t resp_patch_expected[] = { TS_STATUS_CHANGED };

    TEST_ASSERT_BIN_REQ_EXP_BIN(req_patch, sizeof(req_patch), resp_patch_expected,
                                sizeof(resp_patch_expected));
    ...
    uint8_t resp_get_expected[] = { 0x85, /*0x81,*/
                                    0x48, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 };

    TEST_ASSERT_BIN_REQ_EXP_BIN(req_get, sizeof(req_get), resp_get_expected,
                                sizeof(resp_get_expected));
}
...
{
    const uint8_t req[] = { TS_PATCH, 0x18, ID_CONF, 0xA1, 0x18, 0x31, 0x05 };
    const uint8_t resp_expected[] = { TS_STATUS_CHANGED };
