...
/* Use ts_shell_alloc()/ ts_shell_free() instead of malloc */
#include "../../apps/shell/ts_shell.h"
/* Append buffers were made a shell element - use them and replace linenoise originals */
#include "../../apps/shell/ts_shell_abuf.h"
 ...
 * understand example. */
void linenoiseAddCompletion(struct linenoiseCompletions *lc, const char *str)
{
    if (lc->len >= LINENOISE_COMPLETION_MAX_COUNT) {
        return;
    }

    char *copy;
    size_t len = strlen(str);
    int ret = ts_shell_alloc(len + 1, 10, (uint8_t **)&copy);
    if (ret != 0) {
        return;
    }
...
void refreshShowHints(struct ts_shell_abuf *ab, struct linenoiseState *l, int plen) {
    char seq[64];
    if (hintsCallback && plen+l->len < l->cols) {
        ...
        if (hint) {
            ...
            ts_shell_abuf_append(ab,seq,strlen(seq));
            ts_shell_abuf_append(ab,hint,hintlen);
            if (color != -1 || bold != 0)
                ts_shell_abuf_append(ab,"\033[0m",4);
...
static void refreshSingleLine(struct linenoiseState *l) {
    ...
    ts_shell_abuf_append(&ab,l->prompt,strlen(l->prompt));
    ts_shell_abuf_append(&ab,buf,len);
    /* Show hits if any. */
...
static void refreshMultiLine(struct linenoiseState *l) {
    ...
    /* Write the prompt and the current buffer content */
    ts_shell_abuf_append(&ab,l->prompt,strlen(l->prompt));
    ts_shell_abuf_append(&ab,l->buf,l->len);
...
char *linenoise(const char *prompt) {
    char *buf;
    int ret = ts_shell_alloc(max_cmdline_length, 10, (uint8_t **)&buf);
    if (ret != 0) {
        return NULL;
    }
    int count = 0;
